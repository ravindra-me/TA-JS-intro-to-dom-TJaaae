...
allHrsArray.forEach(element => element.style.border = "dashed");
...
document.querySelector('.two').append(p);
...
let box3P = document.createElement('p').innerText = "Prepend inserts as first child"
box3.prepend(box3P)
...
for(let i= 0 ; i < box.length ; i += 2){
  box[i].style.background="aliceblue";
}
...
boxSeven.querySelectorAll('hr').forEach(element => element.remove());
